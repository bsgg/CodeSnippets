...
namespace Utility.DrawTool
{
    public class DrawTool : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            m_RenderTextureSplat = new RenderTexture(1024, 1024, 0, RenderTextureFormat.ARGBFloat);
            ...
            if (Input.GetKey(KeyCode.Mouse0))
            {
                ...
                if (Physics.Raycast(m_Camera.ScreenPointToRay(Input.mousePosition), out m_Hit))
                {
                    if (m_Hit.collider.tag == "Picture")
                    {
                        ...
                        RenderTexture temp = RenderTexture.GetTemporary(m_RenderTextureSplat.width, m_RenderTextureSplat.height, 0, RenderTextureFormat.ARGBFloat);
